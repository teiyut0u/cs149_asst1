...
extern void sqrtSerialAVX2(int N, float initialGuess, float values[],
                           float output[]);
...
int main() {
  ...
  double minSerialAVX2 = 1e30;
  for (int i = 0; i < 3; ++i) {
    double startTime = CycleTimer::currentSeconds();
    sqrtSerialAVX2(N, initialGuess, values, output);
    double endTime = CycleTimer::currentSeconds();
    minSerialAVX2 = std::min(minSerial, endTime - startTime);
  }

  printf("[sqrt serial avx2]:\t[%.3f] ms\n", minSerialAVX2 * 1000);
