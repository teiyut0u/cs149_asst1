...
void clampedExpVector(float *values, int *exponents, float *output, int N) {
  ...
  if (N < VECTOR_WIDTH) {
    return clampedExpSerial(values, exponents, output, N);
  }

  __cs149_vec_float vec_values, vec_output, vec_res,
      vec_threshold = _cs149_vset_float(9.999999f);
  __cs149_vec_int vec_exponents, vec_zero = _cs149_vset_int(0),
                                 vec_one = _cs149_vset_int(1);
  __cs149_mask maskAll = _cs149_init_ones();

  int start = N % VECTOR_WIDTH;
  for (int i = start; i < N; i += VECTOR_WIDTH) {
    __cs149_mask maskZero = _cs149_init_ones(0), maskPositive,
                 maskExceed = _cs149_init_ones(0);

    _cs149_vload_float(vec_values, values + i, maskAll);
    _cs149_vload_int(vec_exponents, exponents + i, maskAll);

    _cs149_veq_int(maskZero, vec_zero, vec_exponents, maskAll);
    _cs149_vset_float(vec_res, 1.f, maskZero);

    if (_cs149_cntbits(maskZero) != VECTOR_WIDTH) {

      maskPositive = _cs149_mask_not(maskZero);
      _cs149_vload_float(vec_res, values + i, maskPositive);

      _cs149_vsub_int(vec_exponents, vec_exponents, vec_one, maskPositive);
      _cs149_vlt_int(maskPositive, vec_zero, vec_exponents, maskPositive);

      while (_cs149_cntbits(maskPositive)) {
        _cs149_vmult_float(vec_res, vec_res, vec_values, maskPositive);

        _cs149_vsub_int(vec_exponents, vec_exponents, vec_one, maskPositive);
        _cs149_vlt_int(maskPositive, vec_zero, vec_exponents, maskPositive);
      }
      _cs149_vlt_float(maskExceed, vec_threshold, vec_res, maskAll);
      _cs149_vset_float(vec_res, 9.999999f, maskExceed);
    }
    _cs149_vstore_float(output + i, vec_res, maskAll);
  }
  if (start) {
    return clampedExpVector(values, exponents, output, VECTOR_WIDTH);
  }
}
...
float arraySumVector(float *values, int N) {
  ...
  __cs149_vec_float res_vec = _cs149_vset_float(0.f), add_vec;
  __cs149_mask maskAll = _cs149_init_ones();

  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    _cs149_vload_float(add_vec, values + i, maskAll);
    _cs149_vadd_float(res_vec, res_vec, add_vec, maskAll);
  }

  int bits = 0;
  while (VECTOR_WIDTH >> bits) {
    ++bits;
  }
  --bits;
  for (; bits; --bits) {
    _cs149_hadd_float(res_vec, res_vec);
    _cs149_interleave_float(res_vec, res_vec);
  }
